...
import HN_API as HN, FacebookAPI as FB, NLP
	...
	if webhook_type == 'postback':
		for sender_id, postback_payload in postback_events(payload):
			...
			if postback_payload == 'GET_STARTED':
				...
				subscribe('daily', sender_id)
				
			elif postback_payload == 'SUBSCRIBE_WEEKLY_PAYLOAD':
				subscribe('weekly', sender_id)

			elif postback_payload == 'UNSUBSCRIBE':
				unsubscribe(sender_id)
		...
		for sender_id, message in messaging_events(payload):
			...
			try:
				FB.send_message(app.config['PAT'], sender_id, "Just a sec, I'm looking that up...")
...
def subscribe(frequency, sender_id):
	if frequency == 'daily':
		print("User %s subscribed daily"%(sender_id))

	elif frequency == 'weekly':
		print("User %s subscribed weekly"%(sender_id))

def unsubscribe(sender_id):
	print("User %s unsubscribed"%(sender_id))
